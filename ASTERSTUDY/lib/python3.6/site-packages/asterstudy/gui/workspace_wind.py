...
class Workspace_tool(QWidget):
    def __init__(self, astergui, parent=None):
        ...
        # self.BCDialog.setWindowModality(Qt.WindowModal)
...
class Workspace(QWidget):
    ...
    def __init__(self, astergui, parent=None):
        ...
        self.ui = uic.loadUi('/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/windsim.ui')
        # self.matui = uic.loadUi('/home/verney/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/mat.ui')
        ...
        # self.ui.pushButton.clicked.connect(self.getGeoPath)
...
#         self.work_space_tool.ui.pushButton_11.clicked.connect(self.start_computing)
        ...
        # 切换前处理、计算、后处理
        self.ui.tabWidget.currentChanged['int'].connect(self.mian_tab_change)
        ## 后处理
        self.postlay = QtWidgets.QHBoxLayout(self.ui.tab_2)
        button_slice = QtWidgets.QPushButton('切片')
        button_paraview = QtWidgets.QPushButton('打开Paravie模块')
        self.postlay.addWidget(button_slice)
        self.postlay.addWidget(button_paraview)

        button_slice.clicked.connect(self.clip)
        button_paraview.clicked.connect(self.active_pv)
#         # 打开工程
...
#         self.work_space_tool.ui.pushButton_13.clicked.connect(self.continue_to_calculate)
        ...
        # #
        # self.matcheck_splitter = QtWidgets.QSplitter(QtCore.Qt.Horizontal, self.ui.tab_3)
        # # self.matcheck_splitter.addWidget(self.ui.horizontalLayout_7)
        # self.v_layout = QtWidgets.QVBoxLayout(self.ui.frame_2)
        # self.matcheck_splitter.addWidget(self.win)
        # self.v_layout.addWidget(self.win)
        # self.win.setVisible(False)
    def clip(self):
        # create a new 'Clip'
        clip1 = Clip(Input=self.res)
        # Properties modified on clip1
        clip1.Scalars = ['POINTS', '']
        # show data in view
        clip1Display = Show(clip1, self.ren_view)
        # trace defaults for the display properties.
        clip1Display.Representation = 'Surface'
        # hide data in view
        Hide(self.res, self.ren_view)
    ...
    def matCheck(self):
        self.matcheckui = uic.loadUi('/home/verney/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/mat.ui')
        self.win = TreeWidget(self.jsonfile)
        self.matcheckui.gridLayout.addWidget(self.win)
        self.matcheckui.show()

       
        ## self.v_layout = QtWidgets.QVBoxLayout(self.ui.frame_3)
        # if self.win:
        #     self.v_layout.removeWidget(self.win)
        #     sip.delete(self.win)
        # self.win = TreeWidget(self.jsonfile)
        # self.v_layout.addWidget(self.win)

        # print('oooo')
        # print(self.ui.frame_2.isVisible())
        # # print(self.ui.frame_2.show())
        # if self.win.isVisible() or self.ui.frame_2.isVisible():
        #     # print(self.ui.frame_2.hide())
        #     print('1')
    ...
    def handleTrans(self):
        ...
        sourcedir = '/home/verney/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/wp/sourceDir/.'

        # os.system('cp ' + self.casPath +' ' + self.workdir)
        # print(projectpath)
        ...
        # infodic['caspath'] = self.casPath
        infodic['startTime'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) 
        
        projectname = self.workdir.split('/')[-1]
        # print(projectname)

        #生成案例的json文件
        self.jsonfile = os.path.join(self.workdir, projectname+'.json')
        print(self.jsonfile)
        ...
        # cmd ='/opt/skyformai/bin/csub  -o output.txt -I -A ICESIM_v1 -Jd ICESIM_v1/' + projectname
        # cmd += ' -n '+self.cores
        # cmd += ' -q '+self.queue
        # cmd += ' -cwd '+self.workdir
        # cmd += (' /home/export/online3/amd_share/anaconda/bin/python3 '
        # # cmd += (' /home/export/online3/amd_share/ParaView-5.8.0-MPI-Linux-Python3.7-64bit/bin/pvpython '
        #         +'/home/export/online3/amd_app/wxy/cas2foam1.0/generate.py '
        #         + self.queue +' '+ self.cores +' '+ self.maxiters +' '
        #         + self.InputInterval +  ' '+self.workdir +' '+ casname)

        # cmd = ('/home/verney/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/Python/bin/python3 '
        #         +'/home/verney/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/cas2foam/transfer.py '
        #         + self.queue +' '+ self.cores +' '+ self.maxiters +' '
        #         + self.InputInterval +  ' '+self.workdir +' '+ self.casname)
        cmd = ('/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/Python/bin/python3 '
                +'/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/wp/mesh-WTSim.py '
        ...
        # self.v_layout.addWidget(self.win)
        # self.win.setVisible(False)
        # from .cas2foam1.generate import generate
        # generate(self.queue, self.cores, self.maxiters ,self.InputInterval ,self.workdir ,casname)
    ...
    def handleCalc(self):
        ...
        # self.receiver = '739509208@qq.com'
        # print(self.workdir, self.windspeed, self.rotatespeed, self.pressure, self.density, self.queue, self.cores, self.timestep, self.maxiters, self.InputInterval, self.receiver)


        parame = {'windspeed':self.windspeed,'atm':self.pressure,'rho':self.density,'nu':self.nu,
        ...
        cmd = ('/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/Python/bin/python3 '
                +'/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/wp/fluidinitial.py '
                + self.workdir +' '+ '"'+str(parame)+'"')
        ...
        try:
            ...
            # self.ui.tabWidget.setEnabled(True)
            # self.show_mesh()
            # self.process.start(self.show_generated_mesh())
            # self.process.finished.connect(self.process.start(self.show_generated_mesh()))
    ...
    def creatProject(self):
        ...
        sourcedir = '/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/wp/sourceDir/.'
...
#         self.log_textBrowser.moveCursor(self.log_textBrowser.textCursor().End)
    ...
    def residual_plotting(self,threadName, delay):
        ...
        while True:
            try:
                time.sleep(delay)
                resdatapath = os.path.join(self.workdir, 'postProcessing/default_fluid/region1/0/residuals.dat')
                print(resdatapath)
                fieldlist = ['h','U','k','epsilon']
                # plotdata = []
                datadic = {}
                for field in fieldlist:
                    datadic[field] = []
                try:
                    with open(resdatapath) as f:
                            ...
                            try:
                                ...
                                if line[0].strip() != '# Residuals' and line[0].strip() != '# Time':
                                    for i, field in zip(range(1,7),fieldlist):
                                        datadic[field].append(eval(line[i].strip()))
                                        # print(i,field)
                                        # pointdic = {}
                                        # pointdic['xvalue'] = int(line[0].strip())
                                        # pointdic['yvalue'] = eval(line[i].strip())
                                        # pointdic['category'] = field
                                        # plotdata.append(pointdic)
                            except:
                                pass
                except:
                    pass
                ...
                if os.path.exists(resdatapath):
                    ...
                    #     key = datadic[key]
                    #     print(key)
                    #     self.residual_plot.curve1.setData(key) 
                    # self.residual_plot.curve1.setData(datadic['h'])
                    # self.residual_plot.curve2.setData(datadic['U'])
                    ...
                    if len(datadic['h']) >= eval(self.maxiters):
                        try:
                            self.ui.tabWidget.setEnabled(True)
                            break
                        except Exception as e:
