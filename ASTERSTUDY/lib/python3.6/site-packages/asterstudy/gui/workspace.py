...
class Workspace_tool(QWidget):
    def __init__(self, astergui, parent=None):
        ...
        self.ui = uic.loadUi('/usr/sw-cluster/simforge/ICESIM/SALOME9.4/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/demo2.ui')
...
class Workspace(QWidget):
    ...
    def __init__(self, astergui, parent=None):
        ...
        self.ui = uic.loadUi('/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/demo3.ui')
        ...
        self.ui.label_11.setMinimumSize(QtCore.QSize(0, 0))
    ...
    def matCheck(self):
        self.matcheckui = uic.loadUi('/usr/sw-cluster/simforge/ICESIM/SALOME9.4/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/mat.ui')
    ...
    def handleTrans(self):
        try:
            if self.workdir == '' or self.casPath == '':
        ...
        self.casname = self.casPath.split('/')[-1]
        ...
        cmd ='/opt/skyformai/bin/csub  -I -A ICESIM -Jd ICESIM_transfer/' + self.projectname
        cmd += ' -n 1'
        cmd += ' -q q_x86_cn_sim'
        cmd += ' -cwd '+self.workdir
        cmd += (' /usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/Python/bin/python3 '
                +'/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/cas2foam/transfer.py '
                + self.workdir +' '+ self.casname)
    ...
    def check_queue(self):
        ...
        self.ui.label_11.setText(str(cores_remain))
        print(cores_remain)
    ...
    def handleCalc(self):
        ...
        cmd ='/opt/skyformai/bin/csub  -I -A ICESIM -Jd ICESIM_solver/' + self.projectname
        cmd += ' -n '+self.cores
        cmd += ' -q '+self.queue
        cmd += ' -cwd '+self.workdir

        cmd += (' /usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/Python/bin/python3 '
                +'/usr/sw-cluster/simforge/ICESIM/SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/cas2foam/submit.py '
                + self.queue +' '+ self.cores +' '+ self.maxiters +' '
                + self.InputInterval + ' ' + self.receiver+' '+self.workdir +' '+ self.casname)
        ...
        try:
            ...
            # self.process.waitForFinished()
            # self.show_generated_mesh()
            self.resdatapath = os.path.join(self.workdir, 'postProcessing/default_fluid/region1/0/residuals.dat')
...
#         self.log_textBrowser.moveCursor(self.log_textBrowser.textCursor().End)
        ...
        while True:
            try:
                ...
                if os.path.exists(os.path.join(self.workdir,'log')):
                    ...
                    epxected_remaining_time = int((total_time-computed_time)*delta_exacuate_time)
                ...
                if os.path.exists(self.resdatapath):
                    ...
                    if len(datadic['h']) >= eval(self.maxiters) and is_over:
                        try:
                            self.ui.tabWidget.setEnabled(True)
                            self.pushButton_stop.setEnabled(False)
                            break
    ...
    def loadinfo(self,infodic):
        ...
        self.casname = os.path.basename(self.casPath)
        self.ui.lineEdit_2.setText(self.workdir)
        self.ui.lineEdit.setText(self.casPath)
        if 'cores' in infodic and os.path.exists(self.workdir + '/processor0'):
            ...
            self.ui.cbox_queue.setCurrentText(self.queue)
            self.ui.cbox_cores.setCurrentText(self.cores)
            import _thread
            try:
