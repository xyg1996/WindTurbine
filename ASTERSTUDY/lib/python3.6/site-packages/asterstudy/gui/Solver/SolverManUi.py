...
class Ui_Sol_Man(QDialog):
    ...
    def collect_data_form_form(self):
        ...
        para_row = self.tableWidget.currentRow()
        para_name = self.tableWidget.cellWidget(para_row,0).text()
        for i in range(self.solver_form.formLayout.rowCount()+2):
            
            item_list = self.solver_form.formLayout.itemAt(i)
            widget_or_layout = item_list.__class__.__name__
            if widget_or_layout == 'QWidgetItem':
                label_or_combo = item_list.widget().__class__.__name__
                if label_or_combo == 'QLabel':
                    value = item_list.widget().text()
                    target_para_list.append(value.rstrip(':'))
                if label_or_combo == 'QLineEdit':
                    value = item_list.widget().text()
                    target_value_list.append(value)
                if label_or_combo == 'QComboBox':
                    value = item_list.widget().currentText()
                    target_value_list.append(value)
            if widget_or_layout == 'QHBoxLayout':
                for j in range(item_list.count()):
                    ...
                    if label_or_combo == 'QLabel':
                        value = item_in_layout.widget().text()
                        target_para_list.append(value.rstrip(':'))
                    if label_or_combo == 'QLineEdit':
                        value = item_in_layout.widget().text()
                        target_value_list.append(value)
